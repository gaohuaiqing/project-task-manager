 ...
 * 获取持久化尺寸
 */
function getPersistedSize(storageKey: string | undefined): { width?: number; height?: number } {
 ...
 * 保存尺寸到 localStorage
 */
function persistSize(storageKey: string | undefined, width?: number, height?: number) {
  if (!storageKey) return;
  ...
  try {
    ...
    console.warn('Failed to persist dialog size:', error);
  }
}
...
export function ResizableDialogContent({
  ...
  useEffect(() => {
    if (!isDragging || dragDirection === null) return;
    ...
    const handleMouseUp = () => {
      ...
      setSize(constrained);
      persistSize(storageKey, constrained.width, constrained.height);
  ...
  }, [isDragging, dragDirection, indicatorSize, constrainSize, storageKey, onResize]);

  // ==================== 重置尺寸 ====================
  const handleResetSize = useCallback(() => {
    const resetSize = constrainSize(defaultWidth, defaultHeight);
    setSize(resetSize);
    persistSize(storageKey, resetSize.width, resetSize.height);
    onResize?.(resetSize.width, resetSize.height);
  }, [defaultWidth, defaultHeight, constrainSize, storageKey, onResize]);
  ...
  return (
    <div className="relative group">
      ...
      {showSizeIndicator && showIndicator && (
        <div className="fixed top-4 right-4 z-50 bg-primary text-primary-foreground px-3 py-2 rounded-lg shadow-lg text-sm font-medium">
      ...
      <button
        className={cn(
          'absolute -top-10 right-0 bg-primary text-primary-foreground px-3 py-1.5 rounded-t-lg text-xs font-medium opacity-0 group-hover:opacity-100 transition-opacity',
