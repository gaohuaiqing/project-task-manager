...
function getSaturationStatus(value: number): SaturationStatus {
  if (value <= 60) return {
    label: '健康',
...
// ✅ 使用 memo 包装成员卡片组件
const MemberCard = memo<MemberCardProps>(({ member, index, animatedValue, saturationStatus, saturationColor }) => {
  return (
    <div
      className={cn(
    ...
    >
      ...
      <div
        ...
        style={{
          background: `radial-gradient(circle at 50% 50%, ${saturationColor}10, transparent 70%)`,
        }}
      />
          ...
          <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2">
                <Badge
                  ...
                  className={cn(
                    ...
                    saturationStatus.bg, saturationStatus.text, saturationStatus.border
                  )}
                >
            ...
            <div className="relative h-2.5 bg-secondary/30 rounded-full overflow-hidden border border-border/30">
              ...
              <div
                ...
                style={{
                  background: `linear-gradient(90deg, transparent, ${saturationColor}20, transparent)`,
                }}
              />
              ...
              <div
                ...
                style={{
                  width: `${animatedValue}%`,
                  background: `linear-gradient(90deg, ${saturationColor}CC, ${saturationColor})`,
                  boxShadow: `0 0 12px ${saturationColor}60`,
              ...
              <div
                ...
                style={{
                  width: `${animatedValue}%`,
                  background: `linear-gradient(90deg, transparent, ${saturationColor}40, transparent)`,
...
// ✅ 使用 React.memo 包装组件，防止不必要的重渲染
export const SaturationChart = memo<SaturationChartProps>(({ members }) => {
  const [animatedValues, setAnimatedValues] = useState<Record<string, number>>({});

  // ✅ 使用 useMemo 缓存成员及其状态，避免每次渲染重新计算
  const membersWithStatus = useMemo(() => {
    return members.map(member => ({
      member,
      saturationStatus: getSaturationStatus(member.saturation),
      saturationColor: getSaturationColor(member.saturation)
    }));
  }, [members]);

  // ✅ 优化动画逻辑：使用 requestAnimationFrame 代替 setTimeout
  useEffect(() => {
    const rafId = requestAnimationFrame(() => {
      const valuesMap: Record<string, number> = {};
      membersWithStatus.forEach(({ member }, index) => {
        // 错开动画时间，减少同时渲染压力
        setTimeout(() => {
          setAnimatedValues(prev => ({ ...prev, [member.id]: member.saturation }));
        }, index * 30);
      });
    });
  ...
  return (
    <Card className="h-full bg-gradient-to-br from-primary/5 to-primary/[0.02] backdrop-blur-xl border border-border/50 rounded-apple-card overflow-hidden">
      ...
      <CardContent className="space-y-3">
        {membersWithStatus.map(({ member, saturationStatus, saturationColor }, index) => (
          <MemberCard
            key={member.id}
            member={member}
            index={index}
            animatedValue={animatedValues[member.id] || 0}
            saturationStatus={saturationStatus}
...
SaturationChart.displayName = 'SaturationChart';
