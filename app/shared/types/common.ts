 ...
 * 数据库时间戳类型
 */
export type DbTimestamp = string; // ISO 8601 格式的字符串

/**
 * 数据库日期类型
 */
export type DbDate = string; // YYYY-MM-DD 格式的字符串
 ...
 */
export function isValidEntityIdList(value: unknown): value is EntityIdList {
 ...
 */
export function toEntityIdList(value: unknown): EntityIdList | null {
  if (isValidEntityIdList(value)) {
    return value;
  }
  if (typeof value === 'string') {
    const ids = value.split(',').map(toEntityId).filter((id): id is EntityId => id !== null);
    if (ids.length > 0) {
      return ids;
    }
  }
